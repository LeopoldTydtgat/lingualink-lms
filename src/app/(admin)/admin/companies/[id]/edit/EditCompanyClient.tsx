...
export default function EditCompanyClient({ company }: { company: Record<string, unknown> }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setTimeout(() => { router.push(`/admin/companies/${id}`); router.refresh() }, 800)
    } catch (err: unknown) {
      ...
      // second click re-PATCHes a record the server has already updated.
      setSaving(false)
    }
  }
  ...
  return (
    <div className="p-6 min-h-full" style={{ backgroundColor: '#f9fafb' }}>
      ...
      <div className="sticky bottom-0 -mx-6 px-6 py-3 border-t bg-white/95 backdrop-blur flex justify-end gap-3"
        ...
        <button
          onClick={() => router.push(`/admin/companies/${id}`)}
