...
export async function dismissStudentWhatsNewItem(itemKey: string): Promise<void> {
  ...
  if (error) {
    console.error('[dismissStudentWhatsNewItem] dismissal upsert failed:', error)
...
// re-fetching drains the feed page by page. The 10-iteration cap is a pure loop
// guard: a failed write now throws on the spot, and exhausting the cap without an
// empty fetch means the feed did not drain — that also throws rather than
...
export async function clearAllStudentWhatsNew(): Promise<void> {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (error) {
      console.error('[clearAllStudentWhatsNew] dismissal upsert failed:', error)
      throw new Error('WHATS_NEW_CLEAR_FAILED')
    }
  }

  if (!drained) {
    console.error('[clearAllStudentWhatsNew] feed still returned items after 10 passes; not drained')
    throw new Error('WHATS_NEW_CLEAR_FAILED')
  }
}
