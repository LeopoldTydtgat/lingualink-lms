-- NEW346 phase 2: refund clawback in complete_report_atomic (applied live 2 Aug 2026)
-- Client rule: student hours follow whether the class happened. A reopened
-- teacher_no_show report corrected to completed or student_no_show re-deducts
-- the refunded hours in the same transaction (guard on hours_refunded, flag
-- flipped back so a later re-correction refunds again). Also widens the
-- lessons cancel guard from <> 'cancelled' to a NOT IN list covering
-- cancelled_by_student and cancelled_by_teacher. CREATE OR REPLACE preserves
-- EXECUTE grants; verified unchanged after apply
-- (postgres/authenticated/service_role, no anon).
-- Live-tested 2 Aug on lesson 30616dad: clawback -1.50 with clean ledger
-- chain, then round-trip re-correction to teacher_no_show re-refunded +1.50.
