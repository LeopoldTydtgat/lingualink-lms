## 8 August 2026 - Closing three stale clock reads on the teacher Upcoming Classes page

Three commits on one surface: 273724d, 8941b06 and bae01ae. They began as a small heading bug and turned into a class of bug. Every place this page read the clock, it read it wrongly, and each one was wrong in a different way.

The first was a timezone mismatch. The day groups were built from a date key resolved in the teacher's account timezone, but the Today and Tomorrow labels above them came from date-fns isToday and isTomorrow, which compare Date objects on the browser's day boundary. A teacher whose device zone differs from their account zone read a heading one day off near midnight, sitting above a list that was grouped perfectly correctly. The fix compares date keys instead of Date objects, so a label and the group it labels are now decided on the same boundary. It reuses two helpers that already existed, the shared local date key builder and a pure UTC string date arithmetic function, so no new library code was added and date-fns left the file entirely.

The second was staleness. Today's date key was captured once when the component mounted, so a tab left open across the teacher's local midnight kept labelling yesterday's group as Today until someone refreshed. I chose a single self-rescheduling timer over polling or focus listeners: it resolves the next local midnight in the teacher's zone through the project's existing wall-time-to-UTC primitive and sleeps exactly until then. The detail that matters is that each hop re-reads the clock rather than incrementing the previous key, which makes an imprecise fire harmless. Firing early leaves the old key in place and reschedules a second later, and a throttled background tab firing late still reads the correct current day on wake. A delay floor stops clock skew turning an early fire into a zero-delay loop.

The third was an impure read gating a real action. The Cancel Class button's 24 hour window was computed from a direct clock read in the render body, which the purity lint rule had been flagging for some time and which only re-evaluated when the card happened to re-render, so a card left open kept offering the button after crossing the boundary. Rather than add a second timer to every card, I widened the shared countdown module to return the signed millisecond lead time it was already computing internally, and derived the gate from the tick each card was already running. That module is the single source of truth for what a lesson says about itself, and it had been withholding the one number a caller needs in order to gate on time, which is exactly why the caller went to the clock itself.

I audited that widening before committing to it, because the module feeds both portals. Four consumers, the type imported in exactly one file, no object literals, mocks or fixtures constructing it anywhere, and every existing test asserting field by field rather than on the whole object. That made the new field additive with no possible compile break and no existing test churn. Six new tests cover it, including both sides of the 24 hour boundary. The suite went from 303 to 309, and the file now passes lint with no errors for the first time.

Every diff was read line by line before staging rather than trusted from a summary, and each commit was checked as discrete hunks against expected line counts to rule out a silent whole-file rewrite.

## 7 August 2026 - Google Calendar busy-sync: mirroring outside commitments into portal availability

The client teaches on other platforms and keeps her whole working life in one Google Calendar. Nothing stopped a student booking a portal slot she had already given away elsewhere. This closes that gap by mirroring her Google calendar into the portal as unavailability, so her outside commitments block bookings automatically. Four commits, 05898b8 through 8f03f47.

I started with a read-only audit rather than a design, and it paid for itself. The audit surfaced that the availability write path does no reconciliation against existing rows, which meant an automated writer would pile duplicates onto manual entries with no way to tell them apart afterwards. That single fact drove the design: a source column on availability, defaulting to manual and constrained to manual or google_sync, giving the sync exclusive ownership of its own rows. DDL applied live through the SQL Editor first, then captured as migration 20260807090000. Added node-ical as an approved dependency because correct RRULE expansion is not something to hand-roll.

The cron route is deliberately single-user. It resolves its target every run as the one profile with the admin role and aborts if that is not exactly one row, so there is no hardcoded account id and no silent guess if the data ever changes. It pulls a private ICS feed over a sixty day window, skips events marked transparent or cancelled at both series and occurrence level, and converts all-day events through the project's shared local-to-UTC primitive in her own timezone rather than reading a bare date as UTC midnight, which would land every all-day block hours off. Overlapping and touching busy periods merge into single rows.

The write path is insert-before-delete, and the ordering is the point. A crash between the two steps leaves both generations in place: duplicate blocks over-block, which costs a bookable slot and nothing else, and the next run clears them. Delete-first would open a window where her outside commitments blocked nothing at all. Every failure mode in the route resolves the same direction, toward over-blocking, and a sync failure never blocks a booking.

My own spec had a hole in it. The delete step was scoped to the same sixty day window as the read, so rows that had already elapsed were never selected for replacement and would have accumulated forever. The agent flagged it, I read the flag instead of skimming it, and the snapshot now takes every sync-owned row with no time filter, making each run a full generational replacement.

Because a silent sync failure looks exactly like a quiet week, the route tracks consecutive failures in the settings table and the teacher Schedule page renders a health banner after three in a row. The banner is admin-gated server-side and fails safe: an unreadable counter shows it, a key that was never written does not, since a cron that has never run is not a cron that is failing.

Verified on production against her real calendar rather than a fixture. First run wrote 138 blocks. Cross-checking a week of them against her actual Google calendar, every timed event, every merged run of back-to-back blocks, and every all-day period lined up at the right local times, including a day where six touching events correctly collapsed into one nine to ten block. Armed the failure counter and confirmed the banner appeared on her Schedule page and on no other teacher's, then cleared it and confirmed it went away. A second run returned 138 written and 138 deleted, proving full replacement with no duplicate accumulation.

One consequence worth stating plainly, because it will look like a bug otherwise: Google is the source of truth for these blocks. Deleting one inside the portal is temporary, since the next run recreates it. Her own manual availability is never selected by the sync and cannot be touched by it.

## 6 August 2026 - Teaching-material homework: assign, watermark, annotate, review

Built and shipped the teaching-material homework feature across all three portals, from schema to UI polish. The client can now hand a student a specific PDF, or a page range of one, as homework. The student works on it in the browser and the teacher reviews the result. Fourteen commits, span 88f32c4 to 23b4b5d, all click-tested before push.

Foundation. A new material_assignments table captured as migration 20260806100000, applied live first through the Supabase SQL Editor. Each row is a grant: sheet, student, optional page range, a homework annotation layer, and a status that supports soft revoke so revoked rows stay behind as an audit trail. RLS carries the access model: teachers assign only their own students through the trainings relationship, students read and update only their own live layer, admin reads everything. The annotations column got a column-scoped UPDATE grant so a student can save their work without being able to touch the grant itself. Added pdf-lib 1.17.1 as an approved dependency for the file work.

File serving. A gated file route is the only way the document reaches a browser. The caller's own RLS select on material_assignments is the sole grant check, then the route verifies the sheet is active and resolves the attachment by name with a charset check. pdf-lib rebuilds the PDF on every request, copying only the granted page range and stamping a diagonal per-page watermark with the student's name. An unsatisfiable page range fails closed with a 422, and the response is inline with no-store so nothing cacheable or downloadable leaves the server. Later faded the watermark so it reads as a mark, not an obstruction.

Teacher side. A material-assignments API handles list, create with page-range validation, and revoke. The study sheet page gained an assign modal with disabled placeholders for non-PDF attachments and an inline revoke confirm. The class report form gained a second entry point: a sheet picker that hands over to the same modal with the report's student preselected, so assigning homework mid-report is two clicks. Teachers also got a read-only Homework section on their student detail page with a gated viewer sub-route, guarded by a new RLS teacher SELECT policy as the relationship gate, deliberately with no write path.

Student side. The Study tab lists material homework as flat cards with a page-range badge, using an RLS-scoped grants query plus a service-role title read because the underlying sheets are staff-audience and invisible to the student role by design. The homework page wraps the existing PDF annotator: a thin wrapper translates between served page indices and real document pages, marks that fall outside the granted range are held aside and merged back into every save so nothing is silently lost, and autosave writes through the RLS-gated homework layer update. Assignment triggers a branded email with deliberately generic copy carrying no file details, dispatched best-effort so an email failure never alters the API response, and rate-limited per user.

Admin side. An admin assign entry point mirrors the teacher route as a service-role handler behind requireAdmin, sharing a preflight helper with the teacher route so validation cannot drift between the two. The admin student detail page gained a Homework section with an assign modal and revoke. Click-tested the full loop: assign returns 201 with a live row and the email arrives, a duplicate assign is refused with a 409 telling the admin to revoke the existing grant first, revoke flips the badge, and the student's Study tab reflects both states.

Polish. Primary styling on the assign button, the file dropdown hidden when a sheet has exactly one PDF with auto-select kept, revoked rows collapsed behind a show-revoked toggle, and the study-sheet assignments table capped to the five newest behind a show-all toggle.

The feature deliberately reopens a boundary that previously kept staff-audience material away from students entirely, at the client's request, and the design keeps that reopening as narrow as the requirement allows: one granted file, one page range, watermarked, view-only, per student, revocable.

## Session 225 - 19 July 2026 - Study Library rebuild complete (NEW345, steps 1 to 7)
### What was built
Rebuilt the entire study library from schema to teardown across seven steps and roughly thirty commits. The old model, a single admin-published sheet list with exercises stored in their own table and completion tracked in a parallel exercise_completions table, is gone. The new model separates ownership, audience, and activity type: sheets carry an owner_id and an audience column (staff or student), teachers author private teaching materials in their own library, and admins publish student worksheets with tiered access.
Step 1 laid the schema: owner_id on study_sheets with a read-tier owner rule, a tags vocabulary with sheet_tags, and the canonical activities and activity_attempts tables with a column grant carve-out so answer keys never reach user-scoped queries. Three pre-conditions were captured as migrations first: the owner clause mirrored into the library-file route tier check, the lesson_annotations owner clause, and the teacher purge preflight counting owned sheets so deleting a teacher can never silently destroy their material.
Step 2 built server-graded MCQs: the student answers in a player that never sees the answer key, and a server route grades the attempt. Step 3 added the admin activity builder with tag management and a per-sheet tag picker. Step 4 was the largest block: the full teacher library, with a tab split between private teaching materials and student worksheets, owned-file upload and delete, duplicate to my library, an Add Resource modal with staged uploads, an assign-to-students flow with scope checks and dedupe, per-student assignment history badges, and a View Responses drill-down. Step 5 added a second activity type, writing_task, with type-discriminated authoring routes, a student submission player, and a teacher review queue with inline feedback. Step 6 migrated completion itself: a marked_done_at column on assignments replaced the legacy mark-done path, every student-facing completion display moved onto the activities system through one shared helper, and the empty-sheet gate was redefined so activities-only sheets became assignable. Step 7 tore the legacy out: the exercises and exercise_completions tables dropped, the old exercise-complete route, legacy create form, and admin modal Exercises tab deleted, the upload route moved onto the shared requireAdmin helper, and dead publicUrl generation removed since the private bucket serves files only through the proxy route.
### Break/Fix Log
- Assigned homework could never register complete: the activity players were not passing assignment_id through to the submit routes, so every attempt landed with a null assignment. Wiring the id from the URL through both players closed it.
- A created sheet lost its difficulty default: sending difficulty explicitly as null blocked the NOT NULL DEFAULT 1 column default. Omitting the key when absent fixed it, caught in live testing.
- The teacher library admin check missed school_admin accounts because it tested role only. Extended to the account_types array.
- Editing a sheet could silently wipe its exercises if the edit-mode load failed: the save was a delete-then-reinsert, so a failed read followed by a save destroyed rows. Save was blocked behind load-success flags until the whole mechanism was deleted in step 7.
### Session result
The library is now one coherent system: ownership decides who manages a sheet, audience decides who can see it, tags classify it, and activities with attempts carry all interactive content and completion. Both legacy tables are dropped, every read and write path goes through the new model, and the go-live gate item for the rebuild is closed.
...
- Fixed J9.1: the teacher Pending Reports list was showing a report as soon as the class was booked, with a working Complete Report button, because the bucket filter checked report status alone. A teacher could file and bill a class that had not happened yet. Added a class-started gate (scheduled_at is at or before now) so a pending report only surfaces once the class has begun. The database already blocks completion until the class has ended, so this is the visible-layer half of the same guarantee - commit ff47f0a.
- Fixed J9.2 form validation: the 150-character minimum lives correctly on the recap field, but the shared error banner at the bottom of the form duplicated that message under the Additional Details box, where it misleadingly read as if Additional Details was at fault. Removed the duplicate (the recap field already has its own inline counter). Also aligned the inline counter to use the trimmed length so it matches the submit guard exactly, closing a corner case where a recap padded with whitespace could make the submit button silently do nothing - commit 4927f0b.
### Break/Fix Log
Issue 1 (J9.1): Symptom - future classes appeared in Pending Reports with a live Complete Report button the moment they were booked. Cause - pendingReports filtered on status only, and the pending report row is created at booking. Fix - added a hasClassStarted check (scheduled_at at or before now) to the pending filter. Lesson - hiding a card is a UX fix, not a security guarantee; the real block lives server-side in complete_report_atomic, which rejects completion until class end. Verified that guard is still in place before relying on it.
Issue 2 (J9.2): Symptom - a recap that was too short showed a red 150-character error at the bottom of the form, under Additional Details, reading as if that optional field was the problem. Cause - the shared error banner duplicated the recap-length message in a misleading position. Fix - dropped the duplicate banner message for that case and aligned the inline counter to the trimmed length used by the submit guard.
Issue 3 (NEW219, logged for separate work): the teacher billing widget drops an ended-but-unreported class to zero in both Current and Projected, so owed money is invisible in the window between class end and report submission. This is a billing-model question, not a code bug. Captured the corrected model for the client to confirm before any build: expected value enters Projected at booking, stays there until the report settles it, then moves to Current on a billable outcome (never creating new projected income), and leaves Projected entirely on a non-billable outcome.
Issue 4 (NEW220, logged for the label pass): completed no-show reports show a green Class taken tag in the teacher portal, because the card keys off status alone and ignores the no-show type. Both student and teacher no-show cases proven. The admin reports table already labels these correctly, so the fix is teacher-portal only and the admin table is the model to copy.
Issue 5 (NEW221, logged for verify-then-fix): reopening a flagged report sets the report back to pending but leaves the lesson at missed, because the reopen action never touches the lesson status. Since billing zeroes a missed lesson, the teacher may stay unpaid even after a valid late submission. Open question for the fix session: whether the completion RPC overwrites missed to completed when the teacher resubmits (its guard only excludes cancelled), which would make this self-healing. Live test case preserved for that check.
### Session result
J9 (report lifecycle) is complete with all eight sub-journeys passing. The three outcome types all behave correctly at the pay level: a normal class and a student no-show both pay the teacher, a teacher no-show pays nothing. The overdue flag, the Missed section, the admin reopen lever, and the admin oversight counts all work as specified. Two display and one billing-model follow-up were surfaced and logged as separate items (NEW219, NEW220, NEW221); none of them blocks J9. Two commits shipped and pushed to dev. Next session: run the NEW221 verification, then proceed to J10 (teacher billing across all statuses and currencies), where the NEW219 model question will come back into focus.
---
