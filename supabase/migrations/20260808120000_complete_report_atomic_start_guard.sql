-- 20260808120000_complete_report_atomic_start_guard.sql
-- Capture of live DDL applied 8 Aug 2026 via the Supabase SQL Editor.
-- Client decision 8 Aug: the class report opens at class START, not class end.
-- All three outcomes (completed, student_no_show, teacher_no_show) are
-- available from the moment the class begins. Supersedes the NEW178 Part 2
-- end-time guard. CREATE OR REPLACE only, never DROP + CREATE: a DROP resets
-- EXECUTE and 403s every teacher report submission.
...
-- Reproduce the live proacl on replay. CREATE OR REPLACE preserves grants on an
-- existing function, so these are no-ops against the current database. On a
-- fresh database they strip the Postgres default PUBLIC grant and restore the
-- exact live set: postgres, authenticated, service_role.
